...
// ─── Shot definitions ─────────────────────────────────────
// Each shot: { name, url, label, auth?, setup?, hero? }
// - auth: 'user' | 'admin' to require a session before this shot
// - setup(page): runs after navigation, before the screenshot
// - hero: true to include this shot in the README gallery (otherwise INDEX-only)
const SHOTS = [
  { name: 'homepage',         url: '/',                          label: 'Homepage with hero and paginated catalog', hero: true },
  ...
  // Auth-required shots
  { name: 'cart-drawer',      url: '/',               auth: 'user',  label: 'Slide-out cart drawer', hero: true,
    async setup(page) { await openCartDrawer(page); } },
  { name: 'wishlist',         url: '/wishlist',       auth: 'user',  label: 'Wishlist with one-click removal' },
  { name: 'checkout',         url: '/checkout',       auth: 'user',  label: 'Checkout with stock-aware quantity controls', hero: true },
  { name: 'orders',           url: '/orders',         auth: 'user',  label: 'Order history with status filters' },
  { name: 'order-detail',     url: '__FIRST_ORDER__', auth: 'user',  label: 'Order detail with downloadable invoice', hero: true },
  { name: 'profile',          url: '/profile',        auth: 'user',  label: 'Tabbed profile (info, address, security)' },

  // Admin-required shots
  { name: 'admin-dashboard',  url: '/admin/dashboard', auth: 'admin', label: 'Admin dashboard: KPIs, top products, revenue chart', hero: true },
...
function updateReadme(captured) {
  ...
  // README shows only hero shots; full set lives in docs/screenshots/INDEX.md
  const heroes = captured.filter((s) => s.hero);
  const indexPath = path.join(OUT_DIR, 'INDEX.md').replace(/\\/g, '/');
  const footer = heroes.length < captured.length
    ? `\n[See all ${captured.length} screenshots →](${indexPath})\n`
    : '';

  const before = src.slice(0, startIdx + README_START.length);
  const after  = src.slice(endIdx);
  const next   = before + '\n\n' + buildGallery(heroes) + footer + '\n' + after;
